...
export const FAMILIES = [
  {
    ...
    note: 'The simplest constraint there is. One flat surface removes half of somewhere they could have been, and everything else in the set is a variation on what that does.',
    members: ['infinite-wall'],
  },
  {
    id: 'two-surfaces',
    n: 'Two surfaces',
    title: 'A corner they are standing inside',
    note: 'Two walls meeting remove three quarters of their reachable space. The constraint is stronger, and it also becomes easier for them to make the situation symmetric again.',
    members: ['corner-case'],
  },
  {
    id: 'visibility',
    ...
    note: 'Here the obstacle does not stop them moving at all. It stops you seeing where they move. These situations behave differently from walls, and confusing the two is the most common way to misread a position.',
  ...
  },
  {
    ...
    note: 'An obstacle with a far side is two edges at once, and how it plays depends entirely on whether it is large or small compared with how far they can move.',
    members: ['the-rock'],
  },
  {
    id: 'vertical',
...
export const APPLICATIONS = {
  ...
  'infinite-wall': {
    predicts: [{
      ...
      options: [
        { label: 'Blue', correct: false, why: 'The wall is behind Red, but from straight on it costs them nothing that it does not also cost Blue. Both screens show the same thing.' },
        { label: 'Neither, it is even', correct: true, why: 'Square to the wall the two screens are identical. Red has lost half their reachable space, but none of the half they lost was going to appear on Blue\'s screen anyway.' },
        { label: 'Red', correct: false, why: 'Red is the constrained one, so they are not ahead. But the constraint is invisible from directly in front, so they are not behind either.' },
      ],
    }, {
      ...
      options: [
        ...
        { label: 'It peaks and then collapses', correct: true, why: 'The advantage is a hump. It rises off the square line, peaks somewhere wide, then falls back to nothing once the wall is cutting your own movement as much as it cuts theirs.' },
      ...
      options: [
        { label: 'It shrinks', correct: true, why: 'The wall was doing the work, and it is no longer close enough to cut their movement. They have bought back most of their reachable space by taking two steps.' },
        { label: 'It is unchanged', correct: false, why: 'Your angle relative to them is the same, but the thing your angle was exploiting has gone.' },
        { label: 'It grows', correct: false, why: 'Nothing about moving them into open ground helps you.' },
      ],
    }],
    ...
    explain: 'Moving off the square line collapses the movement Blue has to cover, because the wall is now edge-on to their view. Push it all the way to 90° and Blue is against the wall too, at which point the two screens match again.',
    rule: {
    ...
    complications: [
      ...
      { label: 'Spread', text: 'Angling shrinks the target as well as the room. Past a point you have made them predictable and too small to hit.' },
    ],
  },
  ...
  'corner-case': {
    predicts: [{
      ...
      options: [
        { label: 'Yes, it is the strongest angle', correct: false, why: 'It is the direction from which their reachable space looks widest, which is the opposite of what you want from it.' },
        { label: 'It is reasonable but not the strongest', correct: true, why: 'It leaves them the most room, but the directions it leaves them are forward and the two forward diagonals, which move slowly relative to your crosshair and are the easiest to track.' },
        { label: 'No, it is the worst place', correct: false, why: 'It is not the strongest angle, but it is far from the worst. The movement it gives them is the movement that is easiest to follow.' },
      ],
    }, {
      question: 'You move round until you are hugging one of the two walls yourself. What have you done?',
      options: [
        { label: 'Maximised the pressure on them', correct: false, why: 'You have maximised the angle, which is not the same thing. What matters is the difference between the two screens, not how far round you walked.' },
        { label: 'Handed them the same constraint you were using', correct: true, why: 'The walls constrain whoever gets close to them. Pressed against one, your own movement is cut just as much, and the fight is even again.' },
    ...
    rule: {
      text: 'Against a cornered enemy the useful angles are small ones. The extremes give the corner back to them.',
      why: 'Both walls constrain whoever gets close to them. Any angle large enough to make their position much worse tends to put you somewhere with the same problem.',
    },
    complications: [
  ...
  'behind-the-corner': {
    predicts: [{
      question: 'Red is fully hidden behind a corner and can step out whenever they like. Where should Blue stand?',
      options: [
        { label: 'Tight against the same wall', correct: false, why: 'It hides Blue too, but it destroys their own room to move and leaves the timing entirely to Red.' },
        ...
        { label: 'Directly in line with the corner', correct: false, why: 'That is the widest view of the space they can appear from, which is the most screen to have to watch.' },
      ],
    }, {
      ...
      question: 'They are standing one metre from the corner. You are six metres from it. Who is forced to commit first?',
      options: [
        { label: 'They are', correct: true, why: 'Every step they take uncovers a large slice of ground at once, so they cannot ease into the angle. You can move in increments they have no way to match.' },
    ...
    manipulate: 'Move Blue further from the corner while leaving Red where they are. Compare how much new ground each player uncovers with the same sideways step.',
    explain: 'Nothing here is about hitting anyone yet. Red\'s hittable area is zero and stays zero until they choose to appear. What Blue is competing for is control over how much gets revealed, and that is decided by distance.',
    rule: {
    ...
    complications: [
      { label: 'Third-person camera', text: 'A chase camera lets the hidden player see round the corner without exposing their body, which removes most of the standoff advantage. Reduce the angle rather than press it. Every figure on this page is first person, so switch the camera in the lab to see it.' },
      { label: 'Spread', text: 'Standoff costs you accuracy on the model when they do appear. If the weapon cannot land at that range, some of the distance has to be given back.' },
    ],
  },

  'the-opening': {
    predicts: [{
      question: 'Red is behind a doorway. Does the doorway restrict how they can move?',
      options: [
        { label: 'Yes, it constrains their movement', correct: false, why: 'Stand them a couple of metres back and the wall is nowhere near them. Their reachable space is completely unrestricted.' },
        { label: 'No, it only restricts what Blue can see', correct: true, why: 'This is a visibility boundary. Their movement is untouched. What the doorway does is decide how much of that movement lands on Blue\'s screen.' },
        { label: 'Both equally', correct: false, why: 'Only if they are standing in the doorway itself. Behind it, the wall is doing nothing to their feet at all.' },
      ],
    }, {
      question: 'You are at 45° with the crosshair on the middle of the doorway. Red pushes through it. What do you have to do?',
      options: [
        { label: 'Flick onto them as they appear', correct: false, why: 'That is what you would need from straight on. From an angle the doorway has already cropped their options down to almost nothing.' },
        { label: 'Almost nothing', correct: true, why: 'They walk into the place you are already aiming. The visible part of their reachable space is barely wider than they are, so there is very little for them to appear in that is not already covered.' },
    ...
    explain: 'From an angle, the doorway crops the visible part of their reachable space down to something close to the size of their body. There is very little screen left for them to appear in that is not already under the crosshair, so pushing through costs them.',
    rule: {
    ...
    complications: [
      ...
      { label: 'Who is further out', text: 'The reveal-control rule applies to the doorway as well. If they are further from the opening than you are, they are managing the exposure and you are the one showing yourself.' },
    ],
  },

  'the-rock': {
    predicts: [{
      question: 'Red is behind a freestanding rock. How many reference directions does their reachable space have?',
      options: [
        ...
        { label: 'None, it is unconstrained', correct: false, why: 'The rock genuinely cuts into their reachable space when they are close to it. The curve is not flat.' },
      ],
    }, {
      ...
      options: [
        { label: 'Yes, always pick a side and hold it', correct: false, why: 'That works when the obstacle is large. Here they change sides faster than you can reposition, so a commitment is just a guess.' },
        { label: 'No, they can switch faster than you can answer', correct: true, why: 'Committing only pays when crossing costs them real time. A small rock does not, so you keep the wider orbit and let them be the one to show.' },
        { label: 'Only if you close the distance first', correct: false, why: 'Closing in makes their crossing cheaper relative to yours, not more expensive.' },
      ],
    }],
    manipulate: 'Enlarge the rock using the variant button, then compare the shape of the curve with the small-rock version.',
    explain: 'A rock much larger than their reachable space stops being one obstacle and becomes two independent corners: it takes them real time to travel from one side to the other, so you can commit to a side. A small rock does not give you that, because they can switch sides faster than you can reposition.',
    rule: {
    ...
    complications: [
      ...
      { label: 'Baiting', text: 'Orbiting at a wider radius than their keeps the reveal-control advantage while still inviting the peek.' },
    ],
  },
  ...
  'high-ground-low': {
    predicts: [{
      ...
      options: [
        { label: 'Nothing, they can already see you', correct: false, why: 'From back on the platform the lip of the floor is in their way, which is the whole reason being close to the wall helped.' },
        { label: 'Come forward to the edge', correct: true, why: 'And at the edge they are showing far more of themselves than you are of yourself, which is the trade the position was built to force.' },
        { label: 'Back away from the edge', correct: false, why: 'Backing off hides them further. It is a good move for them, but it does not get them a shot.' },
      ],
    }],
    ...
    complications: [
      { label: 'Falling', text: 'A player at the edge of a drop treats the edge as solid, because falling is worse than being stopped. That is why the ledge cuts their reachable space at all.' },
  ...
  'high-ground-high': {
    predicts: [{
      ...
      options: [
        { label: 'Most of your body', correct: false, why: 'At that angle the lip of the floor is cutting almost all of you out of their view.' },
        ...
        { label: 'Nothing at all', correct: false, why: 'If they could see nothing there would be no fight. They can see the part of you that is worth the most to them.' },
      ],
    }],
  ...
  'high-ground-corner': {
    predicts: [{
      question: 'Red stands exactly on the corner of a raised platform, where two edges meet. Is that a good place for them?',
      options: [
        { label: 'Yes, they cover both approaches', correct: false, why: 'They can see both, but both edges are cutting their reachable space at once. It is the most constrained point on the platform.' },
        { label: 'No, it is the worst point available', correct: true, why: 'Two edges constrain them simultaneously. They are better off playing around the corner, using each edge in turn, than standing on the place where both apply.' },
      ...
      options: [
        { label: 'It stays where it is', correct: false, why: 'The favourable zone is defined against whichever edge they are using, so it moves when they do.' },
        { label: 'It swings through a right angle', correct: true, why: 'The two edges meet at ninety degrees, so the zone you want jumps from one side of the corner to the other. If you cannot follow it faster than they can walk, they deny you indefinitely.' },
    ...
    manipulate: 'Slide Red along one edge, away from the corner, and watch where the reference direction goes. Then slide them to the other edge.',
    explain: 'Away from the corner it is an ordinary ledge and the reference direction is square to whichever edge they are nearest. As they cross to the other edge, that direction swings through ninety degrees, which means the side of the map you want to be on changes while the fight is happening.',
    rule: {
      text: 'When an enemy can move between two edges, the angle you want moves with them. Play close enough that you can change sides faster than they can.',
      why: 'The favourable zone is defined relative to the edge they are currently using. If you are far away, repositioning takes longer than their walk between edges, and they deny the angle indefinitely.',
    },
    complications: [
  ...
  'the-slope': {
    predicts: [{
      ...
      options: [
        { label: 'Blue, they have a clear view', correct: false, why: 'Blue can see them, but Red\'s movement up and down the slope changes their height on Blue\'s screen far more than Blue\'s movement changes anything for Red.' },
        { label: 'Red', correct: true, why: 'Moving on a slope produces vertical movement on the opponent\'s screen as well as horizontal. Red\'s apparent movement is taller than Blue\'s, and their width is no smaller.' },
    ...
    explain: 'On flat ground the two players\' movement looks the same shape from each other\'s cameras, so only sideways aim correction matters. A slope destroys that: one player\'s steps now move them up and down your screen as well as across it.',
    rule: {
    ...
    complications: [
      ...
      { label: 'Close range', text: 'Close in, a slope also acts as a ledge: approach from the side and it hides your lower body while it cannot hide theirs.' },
    ],
  },
  ...
  symmetrization: {
    predicts: [{
      ...
      options: [
        { label: 'Yes, seeing them is the point', correct: false, why: 'Seeing them is necessary and not sufficient. A clear view of someone who has a clearer view of you is a bad position with good visibility.' },
        { label: 'No, not until you know what they see of you', correct: true, why: 'Advantage is the gap between the two cameras. Until you have looked at the second one, you have half the information and all of the confidence.' },
        { label: 'Only if they are behind cover', correct: false, why: 'Cover is one of the things that creates the gap, but the gap is what you are actually judging.' },
      ],
    }],
    manipulate: 'Load a composite map and solve the advantage field. Then move Red and solve it again.',
    explain: 'Stacked geometry has no closed-form answer, but the field still resolves into a few favourable pockets, and those pockets have the same shapes the individual situations predict: an angle off your own cover, held at a range where the enemy\'s edge is still cutting their movement.',
    rule: {
      text: 'Any advantage you think you have must survive swapping the cameras. If it does not, it is not an advantage.',
      why: 'Almost everything that constrains an opponent constrains you too. What makes a position good is not that their geometry is bad, but that their is worse than yours.',
    },
    complications: [],
  },
};
